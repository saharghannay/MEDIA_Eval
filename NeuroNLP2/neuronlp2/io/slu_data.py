...
def create_alphabets(alphabet_directory, train_path, data_paths=None, max_vocabulary_size=50000, embedd_dict=None,
    ...
    if not os.path.isdir(alphabet_directory):
        ...
        slu_alphabet.save(alphabet_directory)
    else:
        word_alphabet.load(alphabet_directory)
        char_alphabet.load(alphabet_directory)
        slu_alphabet.load(alphabet_directory)
    ...
    logger.info("---SLU Alphabet Size: %d" % slu_alphabet.size())
    return word_alphabet, char_alphabet, slu_alphabet


def read_data(source_path, word_alphabet, char_alphabet, slu_alphabet, max_size=None,
              normalize_digits=False):
    data = [[] for _ in _buckets]
    max_char_length = [0 for _ in _buckets]
  #  print('Reading data from %s' % source_path)
    counter = 0
    reader = SLUReader(source_path, word_alphabet, char_alphabet, slu_alphabet)
      ...
      #  print " sent.words ", sent.word_ids 
       ...
       #     print " bucket_id  ", bucket_id
            if inst_size < bucket_size:
                data[bucket_id].append([sent.word_ids, sent.char_id_seqs, inst.slu_ids])
...
def get_batch(data, batch_size, word_alphabet=None, unk_replace=0.):
    ...
    #    print "  wid_inputs  ", wid_inputs
        ...
        # slu ids
        sid_inputs[b, :inst_size] = sids
        sid_inputs[b, inst_size:] = PAD_ID_TAG
        # masks
    ...
    return wid_inputs, cid_inputs, sid_inputs, masks
...
def iterate_batch(data, batch_size, word_alphabet=None, unk_replace=0., shuffle=False):
    ...
    for bucket_id in bucket_indices:
        ...
        sid_inputs = np.empty([bucket_size, bucket_length], dtype=np.int64)

        masks = np.zeros([bucket_size, bucket_length], dtype=np.float32)
        single = np.zeros([bucket_size, bucket_length], dtype=np.int64)

        for i, inst in enumerate(data[bucket_id]):
            wids, cid_seqs, sids = inst
            inst_size = len(wids)
            # word ids
            ...
            # slu ids
            sid_inputs[i, :inst_size] = sids
            sid_inputs[i, inst_size:] = PAD_ID_TAG
            # masks
        ...
        for start_idx in range(0, bucket_size, batch_size):
            ...
            yield wid_inputs[excerpt], cid_inputs[excerpt], sid_inputs[excerpt], masks[excerpt]


def read_data_to_variable(source_path, word_alphabet, char_alphabet, slu_alphabet, 
                          max_size=None, normalize_digits=True, use_gpu=False, volatile=False):
    data, max_char_length = read_data(source_path, word_alphabet, char_alphabet, slu_alphabet,
   ...
   # print " bucket_sizes ", bucket_sizes
    ...
    for bucket_id in range(len(_buckets)):
        ...
        slu = Variable(torch.from_numpy(sid_inputs))#, volatile=volatile)
        ...
        data_variable.append((words, chars, slu, masks, single, lengths))
...
def get_batch_variable(data, batch_size, unk_replace=0.):
   ...
   # print " bucket_length  ", bucket_length
    words, chars, slu,  masks, single, lengths = data_variable[bucket_id]
    bucket_size = bucket_sizes[bucket_id]
   ...
   # print " index ", index
    ...
    #print " len chars[index] ", len(chars[index])
    return words, chars[index], slu[index], masks[index], lengths[index]
    ...
    for bucket_id in bucket_indices:
        ...
        for start_idx in range(0, bucket_size, batch_size):
            if shuffle:
                ...
                excerpt = slice(start_idx, start_idx + batch_size)
            yield words[excerpt], chars[excerpt], slu[excerpt], masks[excerpt], lengths[excerpt]
