...
embedding=$embeddings_dir"/trained_wiki_media/Cbowwiki_Media_fr_data_iter5_vec_size_300.txt"
embed_type="cbow"
...
for m in LSTM; do
  ...
  for b in {16,32,64}; do
	...
	python SLU_BLSTM_nofreeze.py  --mode $m  --num_epochs 200  --batch_size $b --hidden_size $h  --num_layers $n \
