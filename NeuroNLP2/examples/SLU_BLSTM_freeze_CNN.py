...
from neuronlp2.io import get_logger, conll03_data, CoNLL03Writer, DataWriter, data_loader
from neuronlp2.models import BiRecurrentConv1, BiVarRecurrentConv
...
#    embedding = args.embedding
 #   embedding_path = args.embedding_dict
    ...
    word_alphabet, char_alphabet,  target_alphabet = data_loader.create_alphabets('%s/data_dic'%(data_path), train_path,
    ...
    data_train = data_loader.read_data_to_variable(train_path, word_alphabet, char_alphabet,target_alphabet, use_gpu=use_gpu)
    num_data = sum(data_train[1])
    num_labels = target_alphabet.size()

    data_dev = data_loader.read_data_to_variable(dev_path, word_alphabet, char_alphabet, target_alphabet, use_gpu=use_gpu, volatile=True)
    data_test = data_loader.read_data_to_variable(test_path, word_alphabet, char_alphabet, target_alphabet, use_gpu=use_gpu, volatile=True)
    ...
    if args.dropout == 'std':
        network = BiRecurrentConv1(feat_vec_dim, word_alphabet.size(),
                                  char_dim, char_alphabet.size(),
                                  num_filters, window,
                                  mode, hidden_size, num_layers, num_labels,
                                  tag_space=tag_space, p_rnn=p,bidirectional=bidirectional)
    else:
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        #for batch_train in data_loader.iterate_batch_variable(data_train, batch_size):
            word, features, sents,char, labels, masks, lengths = data_loader.get_batch_variable(data_train, batch_size,unk_replace=unk_replace)
                                                                                      
            optim.zero_grad()
            loss, corr, _ , _ = network.loss(features, char, labels, mask=masks, length=lengths,
	...
	tmp_filename = '%s/predictions/dev_%s_num_layers_%s_%s.txt' % (out_path,args.optim, str(args.num_layers),str(uid))
        ...
        for batch in data_loader.iterate_batch_variable(data_dev, batch_size):
            ...
            _, _, preds, probs = network.loss(features, char, labels, mask=masks, length=lengths,
                                       leading_symbolic=data_loader.NUM_SYMBOLIC_TAGS)
            writer.write(word.data.cpu().numpy(), sents.data.cpu().numpy(),  preds.data.cpu().numpy(), labels.data.cpu().numpy(), lengths.cpu().numpy())
